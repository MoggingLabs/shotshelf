...
| Resizable browse shelf | — | — | — | Landed 2026-08-03, gates-only: the clamps, the programmatic-resize filter and the settings round trip are unit/e2e-gated, but a real drag on a real frameless window — tao's invisible border, the floor stopping the hand, the size surviving a relaunch — has been watched by no human on any OS |
